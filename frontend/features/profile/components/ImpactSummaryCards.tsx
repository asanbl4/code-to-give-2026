...
function getMetricVisual(id: string) {
  if (id === "volunteer-hours") {
    return {
      classes: "border-teal-200 bg-gradient-to-br from-teal-50 to-white text-teal-800",
  ...
  if (id === "milestones") {
    return {
      classes: "border-purple-200 bg-gradient-to-br from-purple-50 to-white text-purple-800",
  ...
  if (id === "donation-count") {
    return {
      classes: "border-orange-200 bg-gradient-to-br from-orange-50 to-white text-orange-800",
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {metrics.map((metric) => {
          ...
          return (
            <article
              key={metric.id}
              className={`relative overflow-hidden rounded-3xl border p-4 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md focus-within:ring-2 focus-within:ring-zinc-300 motion-reduce:transition-none motion-reduce:hover:translate-y-0 ${visual.classes}`}
            >
              ...
              <span className="relative flex h-11 w-11 items-center justify-center rounded-2xl bg-white/90 shadow-sm">
