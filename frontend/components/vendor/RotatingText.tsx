...
export function RotatingText({
  ...
  return (
    <motion.span className={cn("text-rotate", className)} layout transition={transition}>
      {/* The only copy a screen reader sees. The animated characters below are
          hidden, or every rotation would be read out letter by letter. */}
      <span className="text-rotate-sr-only">{texts[index]}</span>
      <AnimatePresence mode="wait" initial={false}>
        <motion.span key={index} className="text-rotate" layout aria-hidden="true">
