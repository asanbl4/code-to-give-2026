...
export function NextMilestone({ milestone }: NextMilestoneProps) {
  return (
    <aside
      ...
      className="rounded-[2rem] border border-highlight/40 bg-gradient-to-br from-highlight-soft to-paper p-5 shadow-sm sm:p-6"
    >
      <div className="flex items-start gap-4">
        <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-paper text-ink shadow-sm ring-1 ring-highlight/40">
        ...
        <div>
          ...
          <h2 id="next-milestone-heading" className="mt-2 text-xl font-semibold text-ink">
            {milestone.label}
          </h2>
        </div>
      </div>
      <div className="mt-5" aria-label={milestone.progressLabel}>
        <div className="flex justify-between gap-3 text-sm font-medium text-ink-soft">
          <span>{milestone.progressLabel}</span>
          <span>Demo only</span>
        </div>
        <div className="mt-2 h-3 overflow-hidden rounded-full bg-paper ring-1 ring-highlight/40">
          <div className="h-3 w-[90%] rounded-full bg-signal" />
        </div>
      </div>
      <p className="mt-4 text-sm leading-6 text-ink-soft">{milestone.encouragement}</p>
    </aside>
  );
}
